...
impl RadixTree {
    ...
    pub fn audit(&self) -> Result<(), String> {
        ...
        for (idx, slot) in self.slots.iter().enumerate() {
            ...
            let covered_chains: HashSet<u32> = state.keys.values().map(|&(c, _)| c).collect();
            if state.chains.iter().copied().collect::<HashSet<u32>>() != covered_chains {
                return Err(format!(
                    "holder {idx} chains index {:?} != covered chains {:?}",
                    state.chains, covered_chains
                ));
            }
