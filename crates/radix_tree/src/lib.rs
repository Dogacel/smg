...
impl FlatTree {
    ...
    pub fn overlap(
        ...
        // one per-query allocation (see `OverlapScratch`). Sized to the
        // full query: a capped hint would just regrow on long chains.
        let mut probes: Vec<RunProbe> = Vec::with_capacity(chain.len());
        for (p, &content) in chain.iter().enumerate() {
