...
use common::{oracle::Oracle, rss_kib, Op, Rng};
...
fn build_ops(rng: &mut Rng, families: &[Family]) -> (Vec<Op>, u64, u64) {
    ...
    for family in families {
        for &holder in &family.holders {
            ...
            // `gap_pct` is a share of BLOCKS; a gap removes exactly one
            // block per family instance, and instances average ~10
            // blocks at these lengths, so the per-INSTANCE chance is
            // 10x the per-block share (capped at "every instance").
            let gap_instance_pct = (profile().gap_pct * 10).min(100);
            if rng.chance(gap_instance_pct) && family.blocks.len() > 2 {
...
fn pinned_workload() {
    ...
    println!(
        ...
        rss_delta,
        rss_delta as f64 * 1024.0 / holder_blocks as f64,
        holder_blocks
    );
    ...
    // never ran).
    let soak_secs: u64 = match std::env::var("RADIX_BENCH_SOAK_SECS") {
        Ok(v) => v
            .parse()
            .unwrap_or_else(|e| panic!("RADIX_BENCH_SOAK_SECS={v:?} is not a u64: {e}")),
        Err(_) => 0,
    };
    if soak_secs > 0 {
