...
/// full re-send.
#[derive(Clone, Default)]
pub struct DigestCache(Arc<Mutex<HashMap<ChainKey, proto::Update>>>);

/// (holder, model, block_size, symbol_kind, tip). `proto::Keyspace`
/// derives neither `Eq` nor `Hash` (prost), so its scalar parts key
/// the map directly.
type ChainKey = (String, String, u32, i32, u64);

fn chain_key(update: &proto::Update, tip: u64) -> ChainKey {
...
impl DigestCache {
    ...
    pub fn resend(&self, holder: &str, tip: u64) -> Vec<proto::Update> {
        self.0
            ...
            .iter()
            .filter(|(key, _)| key.0 == holder && key.4 == tip)
