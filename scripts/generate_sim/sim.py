...
def launch_smgs(profile, logs_dir, smg_bin):
    ...
    for i in range(count):
        ...
        if profile.get("mesh_smgs"):
            ...
            peers = [f"127.0.0.1:{MESH_BASE_PORT + j}" for j in range(count) if j != i]
            cmd += [
                "--enable-mesh",
                "--mesh-host",
                "127.0.0.1",
...
def build_report(run_dir):
    ...
    report = {
        ...
        "drills": {k: v for k, v in meta.items() if k.startswith("index_") or "restart" in k},
    }
...
class Drills:
    ...
    def start_all(self):
        for key, target in (
            ...
            if cfg:
                threading.Thread(target=self._guarded, args=(key, target, cfg), daemon=True).start()
...
def run_profile(profile, run_dir, smg_bin=None, skip_build=False):
    ...
    try:
        ...
        Drills(
            profile, meta, children, children_lock, stop, logs_dir, index_bin, proxies
        ).start_all()
