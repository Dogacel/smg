...
impl PolicyRegistry {
    ...
    pub fn select_worker_with_remote(
        ...
        match remote {
            ...
            // The index is wired but gave this request nothing: the
            // policy must not fall back to its LOCAL prefix state (a
            // partial per-gateway view once a remote index exists), so
            // cache_aware picks on load alone here. With no index wired
            // this is exactly `select_worker`.
            _ if self.remote_index_enabled() => policy.select_worker_remote_only(workers, info),
